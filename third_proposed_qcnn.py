...
def encoding(feature_vector,theta):
...
def W(theta,x):
    ...
    qml.RY(theta[14],wires=2) #new
    qml.RY(2*x[4],wires=0)
    qml.RY(2*x[7],wires=1)
    qml.RY(2*x[1],wires=2)
    qml.RY(theta[15],wires=0)
    qml.RY(theta[16],wires=1)
    #qml.RY(theta[17],wires=2)
    qml.CNOT(wires=[0,1])
    qml.RY(theta[17],wires=1)
    qml.RY(theta[18],wires=2)
    qml.CNOT(wires=[1,2])
    qml.RY(theta[19],wires=2)
    qml.CNOT(wires=[2,1])
    qml.RY(theta[20],wires=1)


# Define the quantum function
@qml.qnode(dev, diff_method = "adjoint")
def circuit(weights,feature_vector):
    encoding(feature_vector,weights[:8])
    W(weights,feature_vector)
    return qml.expval(qml.PauliZ(wires=1))
#import numpy as np
#initial_weights = 2 * np.pi * np.random.random(size=(22))
